...
import { useDeleteDepartmentMutation } from "../../apis/departmentApi";

const DeleteDepartmentNotify = ({ setShowDelete, department }) => {
  const [deleteDepartment] = useDeleteDepartmentMutation()
  const handleDelete = async () => {
    try {
      const response = await deleteDepartment(department?._id).unwrap();
  ...
  return (
    <div className="w-full h-full absolute top-0 left-0 z-20 flex justify-center backdrop-blur-sm items-center">
      ...
      <div className="bg-white z-30 px-10 py-8 rounded-lg lg:w-[30vw]">
        <h1 className="font-[500] text-xl">Delete Department</h1>
        <p className="text-zinc-500 text-sm">Are you sure you want to delete <span className="text-dark font-[500] text-[16px]">{department?.name}</span></p>
...
export default DeleteDepartmentNotify
